    ...
    <div class="box-body">
        {if is_array($api) && array_key_exists('translations', $api) && is_array($api.translations) && array_key_exists($nb_site.api_language_id, $api.translations)}
            {if $edit_language!==null}
                {assign var=url value="{$api.translations[$nb_site.api_language_id].final_url|sprintf:$nb_medioteca.id:$edit_language.language_id}"}
            ...
            {nabu_form method="ajax-post" layout=vertical multiform=":root:language:{if $edit_language!==null}{$edit_language.id}{else}_new{/if}"
                ...
                {nabu_form_fieldset title="{nabu_static key=tit_language}"}
                    {nabu_form_row}
                        {nabu_form_select from=$edit_language field=language_id options=$nb_all_languages options_name=name options_default_name="&lt;Elige un Idioma&gt;" class="col-sm-6" label="{nabu_static key=lbl_language}"}
                ...
                {nabu_form_fieldset title="{nabu_static key=tit_content}"}
                    {nabu_form_row}
                        {nabu_form_textbox from=$edit_language field=title name=title label="{nabu_static key=lbl_title}" class="col-sm-12"}
                    {/nabu_form_row}
                    {nabu_form_row}
                        {nabu_form_textbox from=$edit_language field=subtitle name=subtitle label="{nabu_static key=lbl_subtitle}" class="col-sm-12"}
                    {/nabu_form_row}
                    {nabu_form_row}
                        {nabu_form_textbox type=textarea from=$edit_language field=content label="{nabu_static key=lbl_content}" class="col-sm-12"}
                    {/nabu_form_row}
                {/nabu_form_fieldset}
            {/nabu_form}
        {/if}
